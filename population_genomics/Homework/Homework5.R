...
vcf.filt.indMiss <- read.vcfR("~/Projects/eco_genomics/population_genomics/outputs/vcf_final.filtered.homework.vcf.gz")

#iorginal cuttoff was .75
#second cuttoff was .50 

#filter out by indv. missingness -- higher cutoff is more stringent 
vcf.filt.indMiss <- missing_by_sample(vcf.filt.indMiss,
                                      popmap = meta2,
                                      cutoff=0.50)
...
meta2 <- meta[meta$id %in% colnames(vcf.filt.indMiss@gt[,-1]),]
...
write.csv(vcf.div.MHplot, "~/Projects/eco_genomics/population_genomics/outputs/Gentic_Diff_byRegion5.csv",
          quote=F,
          row.names = F)
